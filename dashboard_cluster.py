...
input_path = str('~/Documents/DynamicPricing/InputFiles/')
#  output_path = str('~/Documents/DynamicPricing/OutputFiles/')
df_lot= pd.read_pickle(input_path + 'lot_level_data_SFR.pkl') 

app.layout = html.Div([
    html.H4('Lakewood Summary Stats by Cluster'), 
    html.Div([
        html.Div([
                html.P('Attribute'), 
                dcc.Dropdown(
...
def display_time_series(attribute, period, n_clusters):
    q = int(quantile)
    #  file_name = period[0:4] + '_hourly.pkl'
    file_name = f'../InputFiles/'
